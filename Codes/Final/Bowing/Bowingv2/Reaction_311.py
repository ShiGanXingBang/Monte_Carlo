...
SAVE_DIR = r"Csv/Bowing_SEM_Reproduction_2026_v3"
...
# 额外几何修正：让 mask/Si 过渡不再是突变直角
MASK_FOOT_ROUND_DEPTH = 18          # mask 脚部预圆化深度
MASK_FOOT_SHRINK_MAX = 10           # mask 脚部最大预收缩
INTERFACE_BAND_DEPTH = 22           # Si 顶部过渡带深度
INTERFACE_BAND_WIDTH = 14           # 从开口侧壁向内的过渡带宽度
...
# ------------------ 角分布与主刻蚀参数 ------------------
ION_SIGMA_DEG = 2.0
NEUTRAL_SIGMA_DEG = 7.0

ION_BASE_ETCH = 0.105
ION_CL_BOOST = 0.13
ION_REFLECT_FACTOR_1 = 0.62
ION_REFLECT_FACTOR_2 = 0.38

# ------------------ Mask 有限刻蚀 + 角点增强 ------------------
MASK_ETCH_BASE = 0.010
MASK_ETCH_ANGLE_GAIN = 0.024
MASK_CORNER_MULTIPLIER = 3.20
MASK_REFLECT_FACTOR_1 = 0.82
MASK_REFLECT_FACTOR_2 = 0.62
ETCH_STEP_MASK = 0.10

# 脚部退缩：只有 mask/Si 交界附近额外增强
MASK_FOOT_BOOST = 2.0
MASK_FOOT_WINDOW = 30

# ------------------ 过渡层（interface band） ------------------
# material=3: 顶部过渡/损伤/混合层，用来消掉突变肩部
TRANSITION_ION_SCALE = 0.72
TRANSITION_NEUTRAL_SCALE = 0.45
TRANSITION_REFLECT_GAIN = 0.10
TRANSITION_ADSORB_GAIN = 0.55

# ------------------ 中性粒子：吸附/反射/化学刻蚀 ------------------
NEUTRAL_REFLECT_BASE = 0.38
NEUTRAL_REFLECT_CL_GAIN = 0.07
NEUTRAL_REFLECT_MASK_GAIN = 0.22
NEUTRAL_REFLECT_CAP = 0.84
ADSORB_BASE = 0.18
MASK_ADSORB_FACTOR = 0.06
MAX_CL_COVERAGE = 4

# ------------------ 深度窗口：上部抑制，中部增强，底部衰减 ------------------
TOP_PASSIVATION_DEPTH = 48.0    # 开口下方这段要更直
MID_BOW_CENTER = 165.0          # 从 mask 底算起的 bowing 中心深度
MID_BOW_WIDTH = 85.0
MID_BOW_GAIN = 0.55
BOTTOM_DECAY_LENGTH = 245.0
BOTTOM_DECAY_STRENGTH = 0.38
...
grid_material = ti.field(dtype=ti.i32, shape=(ROWS, COLS))   # 0=真空, 1=Si, 2=Mask, 3=interface band
        ...
        for i, j in ti.static(ti.ndrange((-1, 2), (-1, 2))):
            ...
            if 0 <= nx < ROWS and 0 <= ny < COLS:
                ...
                if mat == 0:
                    ...
                    has_si_like = 1
        if has_vac == 1 and has_si_like == 1:
            result = 1
    return result
...
def top_passivation_factor(py: int) -> ti.f32:
    ...
    # 开口下方先抑制侧蚀，得到更接近 SEM 的直立上侧壁
    x = depth / TOP_PASSIVATION_DEPTH
    return 0.35 + 0.65 * smoothstep01(x)
...
@ti.func
def is_interface_band_cell(px: int, py: int) -> ti.i32:
    res = 0
    if grid_material[px, py] == 3:
        res = 1
    return res
...
def init_grid():
    ...
    for i, j in grid_exist:
        ...
        if j <= VACUUM_Y:
            ...
            # 在 mask 开口下方两侧建立顶部过渡带，消除 mask/Si 的突变肩部
            for n in range(NUM_TRENCH):
                x_left = LEFT_BORDER + n * (CD + SPACE)
                x_right = RIGHT_BORDER + n * (CD + SPACE)
                local_depth = j - MASK_BOTTOM_Y
                if 0 <= local_depth < INTERFACE_BAND_DEPTH:
                    extra = int((1.0 - float(local_depth) / float(INTERFACE_BAND_DEPTH)) * float(INTERFACE_BAND_WIDTH))
                    if (x_left - extra) <= i <= (x_left + INTERFACE_BAND_WIDTH) or (x_right - INTERFACE_BAND_WIDTH) <= i <= (x_right + extra):
                        grid_material[i, j] = 3
    ...
    for k in range(BATCH_SIZE):
        ...
        while alive and steps < MAX_STEPS:
            ...
            if grid_exist[ipx, ipy] > 0.5:
                ...
                prob_reflect = 0.0
                if is_ion == 1:
                    prob_reflect = 0.10 + 0.72 * (1.0 - cos_theta)
                    if mat == 2:
                        prob_reflect += 0.12
                    elif mat == 3:
                        prob_reflect += TRANSITION_REFLECT_GAIN
                ...
                if did_reflect:
                    ...
                    if is_ion == 1:
                        ...
                        if mat == 1 or mat == 3:
                            ...
                            if mat == 3:
                                prob_etch *= TRANSITION_NEUTRAL_SCALE
                        ...
                        if ti.random() < prob_etch:
                            ...
                            if mat == 2:
                                ...
                                prob_adsorb *= TRANSITION_ADSORB_GAIN

                            prob_adsorb = ti.max(0.0, prob_adsorb)
                            if (grid_material[ipx, ipy] == 1 or grid_material[ipx, ipy] == 3) and grid_count_cl[ipx, ipy] < MAX_CL_COVERAGE:
...
def plot_state(ax, exist_data, mat_data, history_lines, current_count):
    ...
    ax.set_ylim(y_max, 0)
    ax.set_title(f"SEM-oriented Bowing Reproduction v3: {current_count}/{TOTAL_PARTICLES}")
...
def main():
    ...
    print(">>> 模拟开始：SEM 导向复现版 v3（过渡带 + 脚部退缩 + 深度窗口） <<<")
