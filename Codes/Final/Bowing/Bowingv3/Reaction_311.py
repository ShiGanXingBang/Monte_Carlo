...
SAVE_DIR = r"Csv/Bowing_SEM_Reproduction_2026_v4"
...
# mask 几何：只做轻微脚部圆化，不再引入额外“第三材料层”
MASK_FOOT_ROUND_DEPTH = 12
MASK_FOOT_SHRINK_MAX = 5
...
# ------------------ 角分布与主刻蚀参数 ------------------
ION_SIGMA_DEG = 1.8
NEUTRAL_SIGMA_DEG = 6.2

ION_BASE_ETCH = 0.108
ION_CL_BOOST = 0.11
ION_REFLECT_FACTOR_1 = 0.66
ION_REFLECT_FACTOR_2 = 0.42

# ------------------ mask 有限刻蚀 ------------------
MASK_ETCH_BASE = 0.0065
MASK_ETCH_ANGLE_GAIN = 0.015
MASK_CORNER_MULTIPLIER = 1.9
MASK_REFLECT_FACTOR_1 = 0.86
MASK_REFLECT_FACTOR_2 = 0.66
ETCH_STEP_MASK = 0.08
MASK_FOOT_WINDOW = 16
MASK_FOOT_BOOST = 1.35

# ------------------ 中性粒子：吸附/反射/化学刻蚀 ------------------
NEUTRAL_REFLECT_BASE = 0.42
NEUTRAL_REFLECT_CL_GAIN = 0.06
NEUTRAL_REFLECT_MASK_GAIN = 0.24
NEUTRAL_REFLECT_CAP = 0.86
ADSORB_BASE = 0.16
MASK_ADSORB_FACTOR = 0.05
MAX_CL_COVERAGE = 4

# ------------------ 深度窗口：上部直，中部轻微 bowing，底部回收 ------------------
TOP_PASSIVATION_DEPTH = 65.0
MID_BOW_CENTER = 120.0
MID_BOW_WIDTH = 70.0
MID_BOW_GAIN = 0.28
BOTTOM_DECAY_LENGTH = 240.0
BOTTOM_DECAY_STRENGTH = 0.24

# 开口下方肩部修正：仅在靠近 trench 入口的窄区域轻微修圆，避免硬突变
ENTRY_SMOOTH_DEPTH = 36.0
ENTRY_SMOOTH_WIDTH = 11.0
ENTRY_ION_BOOST = 0.42
ENTRY_ADSORB_REDUCTION = 0.38
...
# ------------------ 显示模式 ------------------
DISPLAY_STYLE = "sem"   # 'sem' or 'material'
SHOW_HISTORY = True
...
@ti.func
def is_near_mask_bottom(py: int) -> ti.i32:
    res = 0
    if MASK_BOTTOM_Y - 2 <= py <= MASK_BOTTOM_Y + MASK_FOOT_WINDOW:
        res = 1
    return res


@ti.func
def mask_etch_prob(cos_theta: ti.f32, is_corner: ti.i32, py: int) -> ti.f32:
    p = MASK_ETCH_BASE + MASK_ETCH_ANGLE_GAIN * (1.0 - cos_theta)
    if is_corner == 1:
        p *= MASK_CORNER_MULTIPLIER
    if is_near_mask_bottom(py) == 1:
        p *= MASK_FOOT_BOOST
    return clamp01(p)


@ti.func
def top_passivation_factor(py: int) -> ti.f32:
    depth = ti.max(0.0, float(py - MASK_BOTTOM_Y))
    x = depth / TOP_PASSIVATION_DEPTH
    return 0.16 + 0.84 * smoothstep01(x)


@ti.func
def mid_bow_factor(py: int) -> ti.f32:
    depth = float(py - MASK_BOTTOM_Y)
    t = (depth - MID_BOW_CENTER) / MID_BOW_WIDTH
    return 1.0 + MID_BOW_GAIN * ti.exp(-t * t)


@ti.func
def bottom_decay_factor(py: int) -> ti.f32:
    d = (float(py) - float(MASK_BOTTOM_Y)) / BOTTOM_DECAY_LENGTH
    d = clamp01(d)
    return 1.0 - BOTTOM_DECAY_STRENGTH * d


@ti.func
def lateral_depth_factor(py: int) -> ti.f32:
    return top_passivation_factor(py) * mid_bow_factor(py) * bottom_decay_factor(py)


@ti.func
def opening_edge_distance(px: int) -> ti.f32:
    dmin = 1e6
    for n in ti.static(range(NUM_TRENCH)):
        x_left = LEFT_BORDER + n * (CD + SPACE)
        x_right = RIGHT_BORDER + n * (CD + SPACE)
        d_l = ti.abs(float(px - x_left))
        d_r = ti.abs(float(px - x_right))
        if d_l < dmin:
            dmin = d_l
        if d_r < dmin:
            dmin = d_r
...
@ti.func
def entry_corner_factor(px: int, py: int) -> ti.f32:
    res = 0.0
    depth = float(py - MASK_BOTTOM_Y)
    if depth >= 0.0 and depth < ENTRY_SMOOTH_DEPTH:
        d = opening_edge_distance(px)
        lateral = clamp01(1.0 - d / ENTRY_SMOOTH_WIDTH)
        vertical = 1.0 - depth / ENTRY_SMOOTH_DEPTH
        res = lateral * vertical
    return res
    ...
    for i, j in grid_exist:
        ...
        if j <= VACUUM_Y:
            ...
            for n in range(NUM_TRENCH):
                ...
                local_shrink = 0
                if MASK_BOTTOM_Y - MASK_FOOT_ROUND_DEPTH <= j < MASK_BOTTOM_Y:
                    frac = float(j - (MASK_BOTTOM_Y - MASK_FOOT_ROUND_DEPTH)) / float(MASK_FOOT_ROUND_DEPTH)
                    local_shrink = int(MASK_FOOT_SHRINK_MAX * ti.sqrt(ti.max(0.0, frac)))
    ...
    for k in range(BATCH_SIZE):
        ...
        while alive and steps < MAX_STEPS:
            ...
            if grid_exist[ipx, ipy] > 0.5:
                ...
                prob_reflect = 0.0
                if is_ion == 1:
                    prob_reflect = 0.12 + 0.72 * (1.0 - cos_theta)
                    if mat == 2:
                        prob_reflect += 0.14
                ...
                if did_reflect:
                    ...
                    depth_mod = lateral_depth_factor(ipy)
                    entry_mod = entry_corner_factor(ipx, ipy)

                    if is_ion == 1:
                        if mat == 1:
                            angle_factor = 0.60 + 0.40 * cos_theta
                            prob_etch = ION_BASE_ETCH * angle_factor
                            prob_etch += ION_CL_BOOST * (cl_n / float(MAX_CL_COVERAGE))
                            prob_etch *= depth_mod
                            prob_etch *= (1.0 + ENTRY_ION_BOOST * entry_mod)
                            ...
                            prob_mask = mask_etch_prob(cos_theta, corner_flag, ipy)
                        ...
                        prob_etch = 0.0
                        if mat == 1:
                            prob_etch = neutral_etch_prob(cl_n) * depth_mod
                        ...
                        if ti.random() < prob_etch:
                            ...
                            prob_adsorb *= top_passivation_factor(ipy)
                            prob_adsorb *= (1.0 - ENTRY_ADSORB_REDUCTION * entry_mod)
                            if mat == 2:
...
# 7. 可视化辅助
# ============================================================
def build_material_rgb(exist_data, mat_data):
...
def build_sem_gray(exist_data, mat_data):
    # 仅用于显示风格；不改变物理仿真
    img = np.zeros((ROWS, COLS), dtype=np.float32)
    ygrad = np.tile(np.linspace(0.0, 1.0, COLS, dtype=np.float32), (ROWS, 1))

    vac_mask = exist_data < 0.5
    mask_mask = (exist_data >= 0.5) & (mat_data == 2)
    si_mask = (exist_data >= 0.5) & (mat_data == 1)

    img[vac_mask] = 0.10 - 0.05 * ygrad[vac_mask]
    img[mask_mask] = 0.76 - 0.08 * ygrad[mask_mask]
    img[si_mask] = 0.52 + 0.10 * ygrad[si_mask]
...
def plot_state(ax, exist_data, mat_data, history_lines, current_count):
    ...
    if DISPLAY_STYLE == "sem":
        img = build_sem_gray(exist_data, mat_data)
        ax.imshow(np.transpose(img, (1, 0)), origin='upper', cmap='gray', vmin=0.0, vmax=1.0)
    else:
        rgb = build_material_rgb(exist_data, mat_data)
    ...
    for idx, contours_in_frame in enumerate(history_lines):
        if not SHOW_HISTORY and idx != len(history_lines) - 1:
            continue

        if DISPLAY_STYLE == "sem":
            alpha = 0.08 + 0.28 * (idx / n_hist)
            color = 'white' if idx == len(history_lines) - 1 else '#C8C8C8'
            lw = 1.6 if idx == len(history_lines) - 1 else 0.45
        else:
            alpha = 0.22 + 0.78 * (idx / n_hist)
            color = 'white' if idx == len(history_lines) - 1 else 'red'
            lw = 1.5 if idx == len(history_lines) - 1 else 0.7
    ...
    ax.set_ylim(y_max, 0)
    ax.set_title(f"SEM-oriented Bowing Reproduction v4: {current_count}/{TOTAL_PARTICLES}")
...
def main():
    ...
    print(">>> 模拟开始：SEM 导向复现版 v4（去掉伪过渡层 + 肩部平滑 + 灰度显示） <<<")
