"""
我尝试先把那个轮廓线出问题的原因解决一下，解决了2026.3.11
"""
...
def get_contours(raw_grid):
    ...
    if HAS_SKIMAGE:
        ...
        for contour in contours:
            one_contour = []
            for p in contour:
                # skimage 返回 (row, col) = (y, x)
                one_contour.append((p[1], p[0]))
            if len(one_contour) > 2:
...
def main():
        ...
        if i % 50 == 0: 
            ...
            for idx, contours_in_frame in enumerate(history_lines):
                ...
                for contour in contours_in_frame:
                    hx = [p[0] for p in contour]
                    hy = [p[1] for p in contour]
    ...
    ti.sync()
    final_points = get_contours(grid_exist.to_numpy())
