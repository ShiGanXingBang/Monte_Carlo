...
# 轨迹记录：记录粒子经过次数，用于可视化反射路径（GPU 内累加）
trace = ti.field(dtype=ti.i32, shape=(ROWS, COLS))
...
def simulate_batch():
    ...
    for k in range(BATCH_SIZE):
        ...
        while alive and steps < 2000:
            ...
            # 记录粒子经过位置到 trace（限制上限防止溢出）
            if 0 <= ipx < ROWS and 0 <= ipy < COLS:
                if trace[ipx, ipy] < 1000:
                    trace[ipx, ipy] += 1
            ...
            if grid_exist[ipx, ipy] > 0.5:
                ...
                # 把 cos 值限制在 [0, 1] 范围（我们只关心 0..90°）以避免数值误差
                if cos_theta < 0.0:
                    cos_theta = 0.0
                elif cos_theta > 1.0:
                    cos_theta = 1.0
                ...
                if ti.random() < etch_prob:
                    ...
                    if alive or is_ion == 1:
                        ...
                        #         angle_else = ti.max(0.0, (cos_theta - threshold) / (math.pi/2 - threshold))
                        #         angle_else = ti.min(1.0, angle_else)
                        #         ref_p = base_prob + angle_else
                        #     else:  # 中性粒子 - 漫反射，随机反射
                        #         ref_p = ti.random()
                        # elif mat == 1:  # Si
                        #     if is_ion == 1:  # 离子
...
def main():
    ...
    for i in range(num_batches):
        ...
        if i % 20 == 0:
            ...
            # 叠加粒子轨迹热图（使用对数刻度以增强低计数可见性）
            if trace_data.max() > 0:
                norm = np.log1p(trace_data.astype(np.float32))
                if norm.max() > 0:
                    norm = norm / norm.max()
                    ax.imshow(norm.T, cmap='hot', alpha=0.6, origin='upper', vmin=0, vmax=1)
